...
const Header: React.FC = () => {
  ...
  return (
    <motion.header
      ...
      className="py-4 px-2 sm:px-6 mb-8 flex justify-between items-center relative"
    >
      <div className="flex items-center gap-3">
        ...
        >
          ...
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden relative border-2 border-amber-600 dark:border-amber-500 bg-white">
            <img 
              src="/logo.jpg"
              alt="Biryani"
      ...
      {user ? (
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => signOut()}
            className="text-sm flex items-center gap-1 px-2 sm:px-3 py-1 rounded-md bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 transition"
          >
            <LogOut size={14} />
        ...
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="px-4 py-1.5 bg-amber-600 text-white rounded-md text-sm font-medium hover:bg-amber-700 transition"
        >
          Sign In
        </motion.button>
      )}
